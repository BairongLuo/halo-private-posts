...
- 已加密再次保存草稿：只更新草稿快照,**不重算密文**(公开页继续显示已发布版本)。仅当本次同时改了标题/slug/摘要等公开 metadata 或访问密码时,才走服务端恢复链路,并按已发布快照重算密文。
...
- 已加密再次保存：必须走服务端恢复链路按已发布快照重算 bundle，并同步新的公开 metadata。
...
4. 设置面板保存元数据,改标题/slug/摘要或密码后,已加密文章按已发布快照重算密文并同步公开 metadata。
