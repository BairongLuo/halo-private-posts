...
- 后台登录后，点击文章列表里的“未加锁”状态标签，也会跳转到编辑页自动打开独立面板
...
1. 安装并启用插件。
   预期：文章列表三点菜单中出现与“设置”同级的“文章加密”入口；点击后跳到对应文章编辑页并直接打开独立面板。文章列表中的“已加锁 / 未加锁”状态标签也应可点击进入同一面板。编辑页顶部也应可见“文章加密”入口。后台菜单不再额外暴露私密文章页面。

2. 新建一篇公开文章并保存正文。
   预期：未加锁时原文章页仍是普通正文。

3. 在文章编辑页顶部点击“文章加密”，输入访问口令并点击“根据当前正文加锁”。
