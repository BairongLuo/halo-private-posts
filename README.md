...
- 作者既可以从文章列表三点菜单中与“设置”同级的“文章加密”入口进入，也可以在文章编辑页顶部点击“文章加密”；两处都会进入独立面板
...
- `/console/private-posts` 只作为隐藏的后台恢复兜底页保留；它不再出现在菜单里，主入口回到文章列表三点菜单和文章编辑页

## 已实现能力

### 文章流集成

- 基于 `AnnotationSetting` 维护内部注解字段，以及文章列表三点菜单/编辑页顶部两处“文章加密”入口
